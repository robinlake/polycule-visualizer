...
import {miserables} from "@d3/force-directed-graph"
...
function ForceDirectedGraph() {
    ...
    useEffect(() => {
        ...
        setNodes(Array.from(myData.nodes))
        setLinks(Array.from(myData.links))
        const codes = myData.nodes.map(node => node.code)
        setCountryCodes(codes)
        // ForceGraph(data.nodes, data.links)
    }, [])
    ...
    useEffect(() => {
        if (nodes !== undefined && links !== undefined) {
            ...
            // ForceGraph(countryCodes, links);
        }
    }, [nodes, links])

    return (
        <div>
            force direccted graph
        </div>
    )
}
